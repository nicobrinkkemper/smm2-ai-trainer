...
# Automatically push the generated GGUF file to HuggingFace to completely bypass SSH transfers
print("Pushing GGUF to HuggingFace Hub...")
try:
    api = HfApi()
    
    # We must have the HF_TOKEN environment variable set for this to work
    token = os.environ.get("HF_TOKEN")
    if not token:
        print("ERROR: HF_TOKEN environment variable not set. Cannot upload to HuggingFace.")
    else:
        # Find the generated GGUF file
        import glob
        gguf_files = glob.glob("smm2-qwen3-coder/*Q4_K_M.gguf")
        
        if gguf_files:
            file_path = gguf_files[0]
            file_name = os.path.basename(file_path)
            repo_id = "Geitje1/smm2-qwen3-coder"
